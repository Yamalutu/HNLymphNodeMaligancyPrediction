...
MRN = 12345678 # patient MRN
ct_dir = 'xxxxx\example\contrast CT'  # CT image folder
print(ct_dir)
pet_dir = 'xxxxx\example\PET'  # PET image folder
struct_dir = 'xxxxxx\example\node_structure'  # lymph node contour folder
...
proc = subprocess.Popen(['Extract_ROIs_RadiomicsFeatures_new5.exe']+[ct_dir,pet_dir,struct_dir,PET_exist],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
exit_code = proc.wait()
...
dir_save_checkpoint_model_PET_CT = 'AllTrainNorm_CT_PET_BN_DP005_ClaW1v3_LR1e5_00331_0.94694_1.00000.h5'
...
radi_prob = subprocess.Popen(['radiomics_prediction.exe']+[treatment_site,struct_dir,PET_exist],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
radi_prob.wait()
print('radiomics prediction is done')

## fuse model
fin_proc = subprocess.Popen(['ER_Fusion.exe']+ [treatment_site, struct_dir,PET_exist],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
fin_proc.wait()
